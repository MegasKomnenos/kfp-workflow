"""CLI for timemixer-new model package.

Provides training, Katib trial execution, pipeline compilation, and Katib
manifest generation.  Pipeline submission and cluster bootstrap are handled
by the parent kfp-workflow CLI.
"""
...
from ..kubeflow.katib import build_experiment_manifest, dump_manifest
...
def cmd_train_run(args) -> None:
    from ..cmapss.experiment import run_dataset_pipeline
    ...
    explicit_params = _coerce_values(json.loads(args.params_json)) if args.params_json else None
...
def cmd_train_katib_trial(args) -> None:
    from ..cmapss.experiment import emit_katib_metrics, run_dataset_pipeline

    spec = load_spec(args.spec) if args.spec else ExperimentSpec.model_validate_json(args.spec_json)
    explicit_params = _coerce_values(json.loads(args.trial_params_json))
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="timemixer-new")
