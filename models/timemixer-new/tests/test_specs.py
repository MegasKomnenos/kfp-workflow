from timemixer_new.cmapss.search_space import builtin_search_space, katib_parameter_specs
from timemixer_new.specs import execution_spec, expand_ablation_cases, load_spec


def test_load_default_spec():
    spec = load_spec("configs/experiments/fd_all_core_default.yaml")
    assert spec.metadata.name == "timemixer-new-fd-all-default"
...
def test_expand_ablation_cases():
    ...
    assert len(cases) == 6
    assert any("feature_mode-sensors_only" in case["name"] for case in cases)


def test_katib_parameter_specs():
    params = builtin_search_space("default")
    katib = katib_parameter_specs(params)
    assert any(item["name"] == "d_model" for item in katib)
    assert any(item["name"] == "lr" for item in katib)
