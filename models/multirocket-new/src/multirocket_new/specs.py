...
FeatureMode = Literal["all", "selected", "selected_sensors"]
ScalingMode = Literal["global", "condition"]
SelectionMetric = Literal["rmse", "score", "hybrid"]
ValMode = Literal["last", "sampled_last", "all_windows"]
...
class RuntimeSpec(BaseModel):
    ...
    pipeline_root: str
    image: str = "multirocket-new:latest"
...
class TrainDefaultsSpec(BaseModel):
    seed: int = 42
    feature_mode: FeatureMode = "selected_sensors"
    scaling_mode: ScalingMode = "global"
    seq_len: int = 50
    train_stride: int = 2
    max_rul: int = 125
    val_frac: float = 0.2
    val_mode: ValMode = "sampled_last"
