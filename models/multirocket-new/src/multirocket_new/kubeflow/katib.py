...
from ..search_space import katib_parameter_specs, resolve_search_space
...
def _trial_parameters(search_space) -> List[Dict[str, str]]:
    return [{"name": param.name, "reference": param.name} for param in search_space]
...
def build_experiment_manifest(spec: ExperimentSpec, dataset: str) -> Dict[str, Any]:
    ...
    if spec.runtime.resources.gpu_limit not in {"0", "", None}:
        container["resources"]["requests"]["nvidia.com/gpu"] = spec.runtime.resources.gpu_request
        container["resources"]["limits"]["nvidia.com/gpu"] = spec.runtime.resources.gpu_limit
