...
def build_pipeline(spec: ExperimentSpec):
    ...
    def validate_spec_component(spec_json: str) -> str:
        from multirocket_new.specs import ExperimentSpec
    ...
    def katib_search_component(spec_json: str, dataset: str) -> str:
        from multirocket_new.kubeflow.katib import launch_and_wait
        from multirocket_new.specs import ExperimentSpec

        payload = ExperimentSpec.model_validate_json(spec_json)
        return json.dumps(launch_and_wait(payload, dataset), sort_keys=True)

    @dsl.component(base_image=base_image)
    def final_train_component(spec_json: str, dataset: str, best_params_json: str) -> str:
        from pathlib import Path

        from multirocket_new.experiment import run_dataset_pipeline
        from multirocket_new.specs import ExperimentSpec

        payload = ExperimentSpec.model_validate_json(spec_json)
        explicit = json.loads(best_params_json) if best_params_json else None
        out_dir = Path(payload.outputs.local_results_dir) / payload.metadata.name / dataset.lower()
    ...
    def ablation_component(spec_json: str, dataset: str, best_params_json: str) -> str:
        ...
        from multirocket_new.experiment import run_ablation_only
        from multirocket_new.specs import ExperimentSpec

        payload = ExperimentSpec.model_validate_json(spec_json)
        out_dir = Path(payload.outputs.local_results_dir) / payload.metadata.name / dataset.lower()
        rows = run_ablation_only(payload, dataset, json.loads(best_params_json), out_dir, kubeflow=True)
        return json.dumps({"dataset": dataset, "ablations": rows}, sort_keys=True)
    ...
    def aggregate_component(
        ...
        return json.dumps(
            {
                "experiment": payload.metadata.name,
