"""SOFTS C-MAPSS model plugin.

Adapts the ``softs_new.cmapss.*`` modules to the ``ModelPlugin`` ABC.
All ML logic lives in the ``softs-new`` package — this file is purely
...
class SOFTSModelConfig(BaseModel):
    """Schema for ``model.config`` accepted by the softs-cmapss plugin."""
...
class SOFTSDatasetConfig(CmapssDatasetSelection):
    """Schema for ``dataset.config`` accepted by the softs-cmapss plugin."""
...
def _build_cfg(spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ``softs_new.cmapss.train.train_model``.
    """
    ...
    cfg.setdefault("d_core", 32)
    cfg.setdefault("d_ff", 256)
    cfg.setdefault("e_layers", 2)
    cfg.setdefault("dropout", 0.1)
    cfg.setdefault("activation", "gelu")
    cfg.setdefault("use_norm", False)
...
class SOFTSCmapssPlugin(ModelPlugin):
    """SOFTS model trained/evaluated on C-MAPSS turbofan degradation data."""
    ...
    def model_config_schema(cls):
        return SOFTSModelConfig

    @classmethod
    def dataset_config_schema(cls):
        return SOFTSDatasetConfig
    ...
    def train(
        ...
        import numpy as np
        import torch
        from softs_new.cmapss.model import configure_device
        from softs_new.cmapss.train import train_model
    ...
    def evaluate(
        ...
        import numpy as np
        import torch
        from softs_new.cmapss.model import build_model, configure_device
        from softs_new.cmapss.train import mae, nasa_score, predict_array, rmse
    ...
    def save_model(
        ...
        try:
            ...
            registry.register_model(
                ...
                uri=str(dst_pt),
                framework="pytorch",
                description=f"SOFTS C-MAPSS {cmapss_fd_summary(normalize_cmapss_fd_entries(spec['dataset'].get('config', {}), context='dataset.config'))}",
                parameters={
    ...
    def predict_loaded(
        ...
        from softs_new.cmapss.train import predict_array
    ...
    def hpo_search_space(
        ...
        if profile == "aggressive":
            return [
                SearchParamSpec(name="d_model", type="categorical", values=[32, 64, 128, 256]),
                SearchParamSpec(name="d_core", type="categorical", values=[8, 16, 32, 64]),
                SearchParamSpec(name="d_ff", type="categorical", values=[64, 128, 256, 512]),
                SearchParamSpec(name="e_layers", type="categorical", values=[1, 2, 3, 4]),
                SearchParamSpec(name="dropout", type="categorical", values=[0.0, 0.1, 0.2, 0.3, 0.4]),
                SearchParamSpec(name="activation", type="categorical", values=["relu", "gelu"]),
                SearchParamSpec(name="use_norm", type="categorical", values=[False, True]),
                SearchParamSpec(name="batch_size", type="categorical", values=[64, 128, 256]),
                SearchParamSpec(name="lr", type="log_float", low=1e-4, high=5e-3),
                SearchParamSpec(name="weight_decay", type="log_float", low=1e-7, high=1e-2),
                SearchParamSpec(name="huber_delta", type="categorical", values=[0.5, 1.0, 2.0, 5.0]),
        ...
        return [
            ...
            SearchParamSpec(name="d_core", type="categorical", values=[16, 32, 64]),
            SearchParamSpec(name="d_ff", type="categorical", values=[128, 256, 512]),
            SearchParamSpec(name="e_layers", type="categorical", values=[1, 2, 3]),
    ...
    def hpo_objective(
        ...
        if not hasattr(self, "_hpo_cache") or self._hpo_cache_key != cache_key:
            ...
            import tempfile
            artifacts_dir = tempfile.mkdtemp(prefix="hpo_trial_softs_")
