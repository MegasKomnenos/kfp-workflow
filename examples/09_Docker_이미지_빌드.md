...
│   ├── 파이프라인/벤치마크 컴파일러/클라이언트
│   ├── 플러그인 시스템
│   └── 서빙 + benchmark runtime
└── mambasl-new (모델 패키지)
    ├── MambaSL 모델 아키텍처
    ├── C-MAPSS 데이터 처리
    └── 학습/평가 로직
...
가장 좋은 방식은 레지스트리에 푸시한 뒤 스펙에서 versioned tag를 참조하는 것입니다. 다만 이 프로젝트의 로컬 K8s 환경은 호스트에서 직접 `ctr`에 접근하지 못하므로, 검증된 fallback은 **helper pod + `nerdctl`** 방식입니다.
...
### 2. containerd socket에 접근하는 helper pod 실행

```bash
kubectl create namespace image-loader || true
...
이 흐름은 학습 파이프라인, KServe 서빙, 벤치마크 워크플로우에서 모두 동일하게 사용합니다.
